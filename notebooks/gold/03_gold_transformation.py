...
def build_patient_summary():
    ...
    appointments, completed appointments, treatments, treatment cost, billed and paid amounts.
    Pre-aggregations are used to avoid duplicate counts from multi-table joins.
    """
    try:
        logger.info("Building gold_patient_summary...")

        # Appointment metrics per patient
        appt_by_patient = df_appointments.groupBy("patient_id").agg(
        ...
        # Treatment metrics per patient
        # Enrich treatments with patient_id through appointments to honor the appointment -> treatment join
        treatments_with_patient = df_treatments.join(
            df_appointments.select("appointment_id", "patient_id"),
            on="appointment_id",
            how="left",
        )

        treatments_by_patient = treatments_with_patient.groupBy("patient_id").agg(
            F.count("*").alias("total_treatments"),
            F.sum(F.coalesce(F.col("treatment_cost"), F.lit(0))).alias("total_treatment_cost"),
        )

        # Billing metrics per patient
        # Follow treatment_id -> appointment_id -> patient_id path to reach the patient grain
        billing_with_patient = df_billing.join(
            df_treatments.select("treatment_id", "appointment_id"),
            on="treatment_id",
            how="left",
        ).join(
            df_appointments.select("appointment_id", "patient_id"),
        ...
        billing_by_patient = billing_with_patient.groupBy("patient_id").agg(
        ...
        # Join all patient-level aggregates with the patient dimension
        patient_summary = (
            df_patients.select("patient_id", "patient_name", "gender", "insurance_provider")
        ...
        # Replace null numeric values with zero after outer joins
        numeric_cols = [
            "total_appointments",
